            ...
            with self.connection.cursor() as cursor:
                ...
                for cluster_data in clustered_data:
                    insert_query = """
                        ...
                        (body, tickets, num_of_tickets, status, organization_id, 
...
class TextClustering:
    ...
    def _generate_cluster_themes(self, texts, labels):
        ...
        for cluster_id in range(self.n_clusters):
            ...
            top_indices = cluster_center.argsort()[-10:][::-1]  # Fixed this line
    ...
    try:
        ...
        for cluster_id, cluster_data in clusters.items():
            ...
            pain_points_data.append({
                ...
                'summary': str(summary),
                'ticket_count': int(len(cluster_data['tickets'])),
                'tickets': cluster_data['tickets'],  # All tickets in the cluster
...
async def main():
    ...
    # You can specify organization_id and limit here
    organization_id = os.getenv("ORGANIZATION_ID")  # Get from environment or set directly
    limit = 100  # Set to None to process all tickets
    
    print("Starting customer support ticket clustering process...")
    
    try:
        result = await process_existing_supabase_data(
            organization_id=organization_id,
            limit=limit
        )
        ...
        print(f"   Total clusters created: {result['total_clusters']}")
        print(f"   Database updated: {result['database_updated']}")
        
        # Print cluster summaries
        print(f"\n📋 Cluster Summaries:")
        for i, cluster in enumerate(result['clusters'], 1):
            print(f"\n{i}. {cluster['theme']} ({cluster['ticket_count']} tickets)")
